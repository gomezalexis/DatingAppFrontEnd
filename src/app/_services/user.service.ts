...
export class UserService {
  ...
  getMessageThread(id: number, recipientId: number) {
    return this.authHttp.get(this.baseUrl + 'users/' + id + '/messages/thread/' + recipientId).map((res: Response) => {
      return res.json();
    }).catch(this.handleError);
  }
  ...
  deleteMessage(id: number, userId: number) {
    return this.authHttp.post(`${this.baseUrl}users/${userId}/messages/${id}`, {}).map(res => {}).catch(this.handleError);
  }
